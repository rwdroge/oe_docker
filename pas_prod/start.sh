...
function startServer() {
    ...
    echo "start.sh: Production PAS instance started..."
    ps -ef
}
...
function stopServer() {
    ...
    echo "start.sh: Production PAS stopped..."
    exit 0
}

function initLicense() {
    echo "Checking for production license"
    if [[ -f /app/license/progress.cfg ]]; then
        echo "Production license found in /app/license, copying to /usr/dlc/progress.cfg"
        cp /app/license/progress.cfg $DLC/progress.cfg
    fi
    if [[ ! -f $DLC/progress.cfg ]]; then
        echo "ERROR: No production license (/usr/dlc/progress.cfg) found, exiting..."
        exit 1
    fi  
    echo "Production license validated, proceeding"
}
