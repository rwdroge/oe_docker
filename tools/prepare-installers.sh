...
usage(){
  echo "Usage: $0 -c <compiler|db_adv|pas_dev|pas_prod> -v <12.8.x> [-b <binariesRoot>]" >&2
}
...
if [[ "$COMPONENT" != "compiler" && "$COMPONENT" != "db_adv" && "$COMPONENT" != "pas_dev" && "$COMPONENT" != "pas_prod" ]]; then
